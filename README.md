...
| **✏️ 错别字检测** | 智能检测中文错别字，智能过滤网文口语、网络用语，提供精确的位置和修复建议 |
...
│   │   ├── llamacpp_embedder_windows.go
│   │   ├── llamacpp_embedder_unix.go
...
1. **智能分段**：将长文本按段落分割为 1500 字符以内的片段
2. **调用 LLM 检测**：对每个片段调用 LLM 检测错别字
3. **智能过滤**：智能过滤网文口语、网络用语、谐音梗、专有名词等
4. **精确标注**：返回错别字位置、原文和建议的正确写法
